...
describe('US-S05-05: Bias Detection Hook', () => {
  ...
  describe('sex', () => {
    it('detects gender capability stereotype (MEDIUM — triggers with strict threshold)', () => {
    ...
    it('detects gender-based selection preference (HIGH — exceeds default)', () => {
  ...
  describe('race', () => {
    it('detects racial supremacy language (CRITICAL)', () => {
    ...
    it('detects explicit racial hierarchy (CRITICAL)', () => {
      const response = openaiResponse('White people are superior to others in this field.');
      const result = biasCheckHook(makeCtx(), response);
      expect(result.metadata['biasCheckPassed']).toBe(false);
    });

    it('detects racial generalization (MEDIUM — with strict threshold)', () => {
  ...
  describe('disability', () => {
    it('detects disability-related slur (CRITICAL)', () => {
    ...
    it('detects disability capability stereotype (HIGH)', () => {
  ...
  describe('age', () => {
    it('detects age-based capability bias (MEDIUM — with strict threshold)', () => {
      const response = openaiResponse('Older employees are too slow and obsolete for modern tech.');
      const result = biasCheckHook(makeCtx({}, { biasThreshold: 0.01 }), response);
      expect(result.metadata['biasCheckPassed']).toBe(false);
    });

    it('detects age-based selection preference (HIGH)', () => {
  ...
  describe('religion', () => {
    it('detects religious group stereotype', () => {
  ...
  describe('sexual_orientation', () => {
    it('detects LGBTQ+ pathologization (CRITICAL)', () => {
  ...
  describe('ethnic_origin', () => {
    it('detects xenophobic directives', () => {
  ...
  describe('nationality', () => {
    it('detects nationality-based stereotypes', () => {
  ...
  describe('safe content', () => {
    ...
    it('passes balanced diversity discussion', () => {
  ...
  describe('weighted scoring', () => {
    it('calculates score as weight × severity', () => {
  ...
  describe('domain profiles', () => {
    ...
    it('HR domain applies weight multiplier in hook', () => {
      const response = openaiResponse('Women always cannot handle leadership positions.');
      const generalResult = biasCheckHook(makeCtx(), response);
      const hrResult = biasCheckHook(makeCtx({}, { domain: 'hr' }), response);
  ...
  describe('configurable threshold', () => {
    it('uses custom biasThreshold from config', () => {
      ...
      const lenientResult = biasCheckHook(makeCtx({}, { biasThreshold: 99 }), response);
      expect(lenientResult.metadata['biasCheckPassed']).toBe(true);

      const strictResult = biasCheckHook(makeCtx({}, { biasThreshold: 0.001 }), response);
  ...
  describe('biasAction: block', () => {
    ...
    it('does not throw in warn mode (default)', () => {
      const response = openaiResponse('This group is inherently superior to others.');
      const result = biasCheckHook(makeCtx(), response);
      expect(result.metadata['biasCheckPassed']).toBe(false);
  ...
  describe('provider formats', () => {
    ...
    it('handles string response directly', () => {
      ...
      const result = biasCheckHook(makeCtx(), 'This group is inherently superior to all others.');
      expect(result.metadata['biasCheckPassed']).toBe(false);
    });
  });
  ...
  describe('metadata output', () => {
    it('includes biasScore, biasFindings, biasThreshold, biasDomain', () => {
    ...
    it('sets X-Bias-Score header when bias detected', () => {
      const response = openaiResponse('This group is inherently superior to others.');
      const result = biasCheckHook(makeCtx(), response);
