...
export default function NewToolPage() {
  ...
  const handleStep1Next = () => {
    if (!state.name.trim() || !state.vendorName.trim()) {
      ...
      if (!state.name.trim()) errs.name = ['Name ist erforderlich'];
      if (!state.vendorName.trim()) errs.vendorName = ['Anbieter ist erforderlich'];
    ...
    saveStep(1, {
      ...
      vendorCountry: state.vendorCountry || undefined,
      vendorUrl: state.vendorUrl || undefined,
  ...
  const handleStep2Next = () => {
    if (!state.purpose.trim() || !state.domain) {
      const errs: Record<string, string[]> = {};
      if (!state.purpose.trim()) errs.purpose = ['Verwendungszweck ist erforderlich'];
      if (!state.domain) errs.domain = ['Anwendungsbereich ist erforderlich'];
      setField({ errors: errs });
      return;
    }
  ...
  const handleStep3Next = () => {
    if (state.dataTypes.length === 0 || state.affectedPersons.length === 0) {
      const errs: Record<string, string[]> = {};
      if (state.dataTypes.length === 0) errs.dataTypes = ['Mindestens ein Datentyp erforderlich'];
      if (state.affectedPersons.length === 0) errs.affectedPersons = ['Mindestens eine Personengruppe erforderlich'];
      setField({ errors: errs });
      return;
    }
    saveStep(3, {
      ...
      vulnerableGroups: state.vulnerableGroups,
    });
  };

  const handleStep4Next = () => {
    if (!state.autonomyLevel) {
      setField({ errors: { autonomyLevel: ['Autonomiestufe ist erforderlich'] } });
      return;
    }
    saveStep(4, {
      autonomyLevel: state.autonomyLevel,
      humanOversight: state.humanOversight,
      affectsNaturalPersons: state.affectsNaturalPersons,
    });
  };
  ...
  const handleClassify = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Fehler bei der Klassifizierung';
      setState((prev) => ({
        ...prev,
        classifying: false,
        errors: { _form: [message] },
  ...
  // Show result after classification
  if (state.classifyResult) {
    return (
      <div className="container mx-auto max-w-3xl px-4 py-8">
        <ClassificationResult
          result={state.classifyResult}
          toolName={state.name}
          onViewTool={() => router.push(`/tools/${state.toolId}`)}
          onBackToInventory={() => router.push('/tools/inventory')}
        />
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-8">
      <h1 className="mb-2 text-2xl font-bold tracking-tight">AI Tool registrieren</h1>
      <p className="mb-6 text-sm text-slate-500">
        Registrieren Sie ein AI Tool und lassen Sie es nach EU AI Act klassifizieren.
      </p>
      ...
      {state.step === 1 && (
        <Step1Tool
          data={{ name: state.name, vendorName: state.vendorName, vendorCountry: state.vendorCountry, vendorUrl: state.vendorUrl, description: state.description }}
      ...
      {state.step === 2 && (
        <Step2Usage
          data={{ purpose: state.purpose, domain: state.domain }}
          onChange={(d) => setField(d)}
      ...
      {state.step === 3 && (
        <Step3Data
          data={{ dataTypes: state.dataTypes, affectedPersons: state.affectedPersons, vulnerableGroups: state.vulnerableGroups }}
          onChange={(d) => setField(d)}
      ...
      {state.step === 4 && (
        <Step4Autonomy
          data={{ autonomyLevel: state.autonomyLevel, humanOversight: state.humanOversight, affectsNaturalPersons: state.affectsNaturalPersons }}
          onChange={(d) => setField(d)}
