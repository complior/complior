...
Status bar (bottom):
  ...
  NORMAL  D:dash S:scan F:fix P:passport O:oblig T:time R:report L:log ?:help
```

8 страниц. Было 8 (Dashboard, Agents, Scan, Fix×2, Report, Log, Orchestrator). 
Убрано 2 (Agents/PTY wrapper, Orchestrator). Добавлено 3 (Passport, Obligations, Timeline).
...
NORMAL  D:dash S:scan F:fix P:passport O:oblig T:time R:report L:log
```
