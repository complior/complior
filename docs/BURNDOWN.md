...
> **Текущее состояние:** 16 спринтов завершено, 3 осталось | 311 SP / ~365 SP (85%)
> **Тесты:** 476 (280 Engine + 9 SDK + 187 TUI) | **User Stories:** 69 / 83
...
| Метрика | Значение |
|---------|----------|
| Спринтов завершено | 16 (Phase 0, E01, T02, T02.5, E03, E04, T03, T04, T05, E05, E06, E06.5, T06, T07, E07) |
| Спринтов осталось | 3 (E08, T08, L09) |
| SP завершено | **311** |
| SP осталось | **54** |
| SP итого | **~365** |
| Тесты Engine | **280** (32 test files, Vitest) |
| Тесты SDK | **9** (2 test files, Vitest) |
| Тесты TUI | **187** (cargo test, Rust) |
| Тесты итого | **476** |
| User Stories завершено | **69** |
| Средняя скорость | **20.7 SP/спринт** |
...
| **Итого** | **311** | **311** | **15 дней** | **20.7 avg** |
...
|---------|----------|--------|----------|
| **Unit / Mock** | Модульные тесты на моках: Vitest (Engine + SDK), `cargo test` (TUI). Проверяют отдельные функции, типы, обработчики | ✅ Активно | 476 тестов (280 Engine + 9 SDK + 187 TUI) |
| **E2E (ручное)** | Запуск реального сервера / бинарника, HTTP-запросы, tmux захват. Проверяет реальное поведение end-to-end | ✅ Проведено | 66 тестов (E2E T06: 43, E2E T07: 13, E2E E07: 20) — всё 100% PASS |
...
**Дата:** 2026-02-19 (E06.5 + E07 added, burndown corrected)
