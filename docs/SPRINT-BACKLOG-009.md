...
**Capacity:** 51 SP (43 новых + 8 carry-over из S8) | **Duration:** 4 недели
...
#### US-105: CLI → SaaS FRIA Sync — structured assessment import (3 SP)

- **Feature:** F70 | **Developer:** Marcus (backend)

##### Описание

Как deployer, после генерации FRIA через CLI (`complior agent fria <name>`) и выполнения `complior sync`, я хочу чтобы результат появился в SaaS как полноценный `FRIAAssessment` с 6 структурированными секциями — не как markdown blob — чтобы я мог продолжить редактирование FRIA в удобном UI, не начиная заново.

> **Контекст:** CLI генерирует FRIA и pre-fills 7 полей из passport. Остальные 29 полей manual. Сейчас FRIA синхронизируется как markdown в `ComplianceDocument.metadata.content` — SaaS не может его структурно обработать. Нужен новый endpoint `POST /api/sync/fria` который принимает structured JSON и создаёт `FRIAAssessment` + `FRIASection` записи.

##### Реализация

**Новые файлы:**
- `app/api/sync/fria.js` — POST `/api/sync/fria` (api auth via Bearer token)
- `app/application/sync/processFria.js` — use case: JSON → FRIAAssessment + 6 FRIASection upsert

**Модифицированные файлы:**
- `server/lib/schemas.js` — `SyncFriaSchema` Zod validation (6 section objects)
- `app/schemas/SyncHistory.js` — добавить `'fria'` в syncType enum
- `app/setup.js` — migration для syncType enum extension

##### Merge Strategy

- **Tool not found** → `{ action: 'skipped', reason: 'tool_not_found' }`
- **No existing assessment** → CREATE FRIAAssessment (draft) + 6 FRIASection
- **Existing draft assessment** → MERGE: CLI fills empty SaaS fields, SaaS non-empty fields preserved
- **Existing in_progress/review assessment** → SKIP: `{ action: 'skipped', reason: 'assessment_in_progress' }`

##### Критерии приёмки

- [ ] `POST /api/sync/fria` с structured JSON → создаёт FRIAAssessment + 6 FRIASection
- [ ] Pre-filled поля из CLI (toolName, vendor, riskLevel, oversight) заполнены в sections
- [ ] Re-sync на draft → merge (CLI fills empty, SaaS non-empty preserved)
- [ ] Re-sync на in_progress/review → skip (SaaS wins)
- [ ] Tool not found → graceful skip
- [ ] SyncHistory запись создаётся для каждого sync
- [ ] Frontend FRIA UI показывает CLI pre-filled данные без изменений в UI коде

- **Tests:** 0 (verification via curl + DB query)
- **Dependencies:** CLI Sync (S8), FRIA (S8)
...
| Phase | Stories | SP |
|-------|---------|-----|
| Foundation + Carry-over | US-091, US-086, US-089, US-105, US-098 | 13 |
...
```
# Phase 1
app/api/sync/fria.js
app/application/sync/processFria.js
