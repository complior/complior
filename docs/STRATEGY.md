...
### Фаза 1: SDK Hardening + Engine Quick Wins (S05, 2 недели)
...
### Фаза 1.7: ISO 42001 Document Generators (V1-M07, 1 неделя) ⏳ IN PROGRESS
...
│                                                                      │
│  NOTE: F-V9-24 (AI Policy) уже реализован через agent policy.       │
│  Результат: complior agent soa|risk-register генерируют              │
│  готовые к аудиту ISO 42001 документы из scan данных.                │
│  T-4/T-5/T-6: 23 tests GREEN, 0 regressions. T-7 awaiting rust-dev.│
│  Branch: feature/V1-M07-iso42001                                     │
└──────────────────────────────────────────────────────────────────────┘
```
...
### Фаза 1.9: Onboarding Enrichment (V1-M09, 1 неделя) ⏳ IN PROGRESS
...
│                                                                      │
│  Проблема: risk-profile.json даёт 15-25 obligations (26%).          │
│  Реальность: 108 obligations, метаданные есть, нужна фильтрация.    │
│  Решение: 9 вопросов + filter obligations.json directly.            │
│  Ожидание: deployer+limited = 16, provider+high+GPAI = 77.         │
│  Branch: feature/V1-M09-onboarding-enrichment                       │
└──────────────────────────────────────────────────────────────────────┘
```

### Фаза 2.0: Score Transparency (V1-M10, 1 неделя) ⏳ PLANNED
...
│  * T-4   fix/report/eval profile-awareness                   ⏳     │
│  * T-5   complior status — single-command posture            ⏳     │
│                                                                      │
│  Проблема: 80/100 = 80% of 39 checks. 69 obligations unchecked.    │
│  Решение: disclaimer + categories + status command.                  │
│  Branch: feature/V1-M10-score-transparency                           │
└──────────────────────────────────────────────────────────────────────┘
```

### Фаза 2.1: Command Restructuring (V1-M11, 1 неделя) ⏳ PLANNED

```
┌─── V1-M11: COMMAND RESTRUCTURING ────────────────────────────────────┐
│                                                                      │
│  `complior agent` → split into passport + fix --doc (8 задач):       │
│  * T-1   Update E2E test URLs (architect — RED specs)         ⏳     │
│  * T-2   Update acceptance scripts (architect)                ⏳     │
...
│                                                                      │
│  Проблема: `complior agent` = passport CRUD + document generation.   │
│  Document generation дублирует `complior fix` (Type B findings).     │
│  Решение: passport = identity mgmt, fix --doc = all doc generation.  │
│  Blast radius: 27 routes, 20 CLI variants, 40+ docs, 5 E2E files.   │
│  Depends on: V1-M07 + V1-M09 merged to dev.                         │
