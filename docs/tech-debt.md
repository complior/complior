...
**Updated:** 2026-05-01
**Author:** Reviewer (V1-M30.8b review — TD-63 added (BLOCKING: 2 superseded tests))
...
| TD-63 | MEDIUM | V1-M30.8b W-4 disclaimer redesign supersedes 2 older tests that checked for file:// links in the excluded-docs disclaimer section. The old `excludedLinks` feature (file:// anchors to excluded documents) was replaced by `excludedLabels` (text-only "Declaration of Conformity (provider-only) and FRIA (Art. 27 — high-risk only)"). Additionally, both tests use FRIA with limited-risk profile — FRIA is excluded since V1-M30.6 and no longer appears in doc-cards. **Architect must fix before merge.** | `v1m30-4-tab-ux.test.ts` (A.4 file:// links), `v1m30-5-doc-links-actions.test.ts` (W-2 relative path resolve) | Both tests GREEN after fixture update (switch FRIA→universally-applicable doc OR profile→high-risk) | 🔴 OPEN (BLOCKING — must fix before merge to dev) |
