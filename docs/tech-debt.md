...
**Updated:** 2026-04-29
**Author:** Reviewer (V1-M30.5 review — TD-60 FIXED, add TD-61 (OPEN/BLOCKING), TD-62 (OPEN/process))
...
- TD-61: Architect's RED test uses `if line.contains(...) { panic!(...) }` but clippy `manual-assert` lint requires `assert!(!line.contains(...), ...)`. This is a 1-line fix in the architect's own file (`tests.rs:2004`). BLOCKING for CI — must be fixed before merge to dev
- TD-62: V1-M30.5 dev modified 2 old V1-M29 test files that directly contradicted the new V1-M30.5 spec: (1) Actions test asserted "no passport init" but W-3 spec removes the filter entirely; dev replaced with positive assertion that all actions render. (2) Documents test asserted "FRIA excluded for limited-risk" but W-2 spec shows FRIA for all profiles; dev flipped the assertion. Both modifications are spec corrections, not weakening — the old tests would fail on the new correct behavior. Process note: dev should file SCOPE VIOLATION REQUEST even for justified corrections. Recurring pattern from TD-52 (V1-M27) and TD-53 (V1-M29)
