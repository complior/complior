...
**Рекомендуемый порядок:** `complior agent init` (паспорт) → `complior scan` → `complior fix` (scaffolds pre-filled из паспорта) → `complior fix --ai` (LLM enrichment). Паспорт — обязательный первый шаг: без него scaffold-документы содержат `[PLACEHOLDER]` вместо реальных данных.
