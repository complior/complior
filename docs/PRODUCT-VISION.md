...
│ │                                                                   │     │
│ │ ┌─────────── 1. ENGINE (TS daemon) ──────────────────────────┐   │     │
│ │ │ Scan: L1-L4 static code analysis (+ --deep: Semgrep/Bandit)│   │     │
│ │ │ Eval: 550 dynamic tests (conformity + security, --target)  │   │     │
│ │ │ Red Team: 300+ OWASP/MITRE attack probes (embedded)        │   │     │
│ │ │ Fixer (18 strategies: A1-A9, B1-B6, C1-C3, D1-D3)        │   │     │
│ │ │ Document Generator (14 EU AI Act templates)                 │   │     │
│ │ │ Passport Service (36 fields, auto-update, ed25519)          │   │     │
...
| **Development** | Deep Scan Tools | uv auto-download | Open-source | Готов | Semgrep + Bandit + ModelScan (~150MB) |
...
- **Mode 1 (Auto)**: CLI анализирует AST → auto-fill 85-95% полей. Production-ready.
- **Mode 2 (Runtime)**: MCP Proxy наблюдает tool calls + Eval тестирует систему → 40-70%. PLANNED.
- **Mode 3 (Manual)**: SaaS wizard + AI Registry pre-fill → 100%. Sprint S09.

Паспорт обновляется автоматически: `scan.completed` → обновляет `complior_score` + `last_scan` (все scan tiers). Планируется: eval → conformity/security scores, monitor → drift status.
...
**Рекомендуемый порядок:** `complior passport init` (паспорт) → `complior scan` → `complior fix` (scaffolds pre-filled из паспорта) → `complior fix --ai` (LLM enrichment). Паспорт — обязательный первый шаг: без него scaffold-документы содержат `[PLACEHOLDER]` вместо реальных данных.
...
│  │  │ Mode 2: RT  │  │ ed25519     │  │ Маппинг: finding→oblg. │ │   │
...
│  │  │/passport│  │ fix_apply │  │ FRIA fill│  │ Drift detect │  │   │
│  │  │/fix/doc │  │           │  │          │  │              │  │   │
...
|---|------|----------|--------|
| E-F1 | **Scanner** | 6 тиров (1/1+/2/2+/3/3+): Offline (L1-L4), +LLM (L5 BYOK), +Deep (Semgrep/Bandit/ModelScan via uv), +Cloud (SBOM/PII/vendor). Score 0-100. Industry patterns. Multi-framework scoring (EU AI Act + AIUC-1 + OWASP + MITRE). Security scoring (OWASP LLM Top 10 + MITRE ATLAS, 300+ probes). Incremental scan (mtime + SHA-256 cache, 10-50x speedup) | Tier 1/1+/2/2+ готов, Tier 3 планируется (cloud) |
| E-F2 | **Fixer** | 18 стратегий: A (код, 9): SDK wrapper, validation, error handling, disclosure, logging, permission guard, HITL gate, config hardening, unsafe deser. B (документы, 6): FRIA, AI Policy, Risk Plan, Tech Docs, Worker Notif, Transparency. C (deps, 3): CVE upgrade, license, model format. D (config, 3): secret rotation, gitignore, Docker security. E (passport, 3): risk level, compliance fields, evidence chain. + Undo | Готов |
...
| E-F4 | **Passport Service** | 36-полевой паспорт ИИ-агента. 3 режима: Auto (AST, production), Runtime (MCP Proxy + Eval, planned), Manual (SaaS Wizard). Auto-update после scan. ed25519 подпись. Export: A2A, AIUC-1, NIST | Готов (Mode 1) |
...
| E-F6 | **FRIA Generator** | Fundamental Rights Impact Assessment. Шаблон + подстановка из паспорта (80% pre-fill). AI Enricher: LLM-дозаполнение (scaffold + enhance modes, L2 feedback) | Готов |
...
| E-F8 | **LLM Module** | Vercel AI SDK. L5 deep analysis, Chat Service (SSE streaming), rate limiter. AI Enricher для FRIA и документов (scaffold + enhance) | Готов |
...
# Agent Passport
complior passport init            # обнаружить AI-системы → сгенерировать passports
complior passport list            # список всех passports
complior passport show <name>     # показать конкретный passport
complior passport validate        # проверить completeness
complior passport export --format a2a|aiuc-1|nist
complior passport evidence        # export evidence chain
complior passport registry        # per-agent compliance dashboard
complior passport permissions     # permissions matrix
complior passport audit           # unified audit trail

# Document Generation (from passport data)
complior fix --doc fria <name>    # генерация FRIA (80% pre-filled из passport)
complior fix --doc notify <name>  # worker notification (Art.26(7))
complior fix --doc policy <name>  # AI usage policy (5 industries)
...
# Eval (динамическое тестирование)
complior eval <url>            # 550 тестов: conformity + security
complior eval <url> --det      # только детерминистические (168)
complior eval <url> --llm      # + LLM-judged (212, BYOK)
complior eval <url> --security # OWASP/MITRE (300 probes)
complior eval <url> --full     # all combined
...
Текущее покрытие: ~65% автоматически, ~25% с шаблонами, ~10% manual (SaaS/monitoring).
...
| Ст. 9 | Управление рисками | Engine (Doc: Risk Register) | Готово |
...
| Clause 6.1.3 | Statement of Applicability | Engine (Doc) | Готово (soa-generator.ts, 39 Annex A controls) |
| A.2.2-2.3 | AI Policy | Engine (Doc) | Готово (шаблон + AI Enricher) |
| Clause 6.1.2 | Risk Register | Engine (Doc) | Готово (risk-register-generator.ts) |
...
**✅ MONTH 1 SCOPE COMPLETE (v1.0.0, April 2026):**
- SDK production: 14 hooks (prohibited 138, sanitize 50+ PII, disclosure, bias, etc.) — **DONE**
- Engine: Scanner L1-L5, Fixer (18 стратегий), 14 документов, Passport (36 полей, ed25519) — **DONE**
...
- `complior scan --deep` — Semgrep/Bandit/ModelScan via Tier 2 integration — **DONE**
- `complior eval <url>` — 11 категорий, 680+ тестов, auto-detect адаптеры, SSE streaming — **DONE**
- ISO 42001 документы (SoA, Risk Register, AI Policy) — generators + templates — **DONE**
- FRIA LLM-дозаполнение (AI Enricher: scaffold + enhance modes) — **DONE**
- Incremental scan (mtime + SHA-256 cache, 10-50x speedup) — **DONE**
- MCP Compliance Proxy (policy engine + proxy interceptor) — **DONE**
- Supply Chain Audit (45+ AI SDKs, CVE detection) — **DONE**
- Dependency Scan (5 package managers, lock file parsing) — **DONE**
- Test Generation (JS/Python/Go output from passport constraints) — **DONE**
- E2E acceptance: 34/34 PASS, unit tests: 202 Rust + 2225 TS = 2427 GREEN — **DONE**
...
```
MONTH 1 — PURE OPEN-SOURCE ✅ COMPLETE (v1.0.0, April 2026)
══════════════════════════════════════════════════════════════════
S05  ████████████████████  SDK+Agent+Cert+Runtime+Multi   ✅ DONE
S06  ████████████████████  ISO 42001, FRIA LLM, MCP Proxy ✅ DONE
S07  ████████████████████  Incremental scan, streaming    ✅ DONE
S08/S09 ████████████████░  Scanner Intelligence           ✅ DONE
S10  ████████████████████  Deep scan + OWASP/MITRE        ✅ DONE
...
MONTH 7+ — PAID
──────────────────────────────────────────────────────────────────
SaaS  ░░░░░░░░░░░░░░░░░░░  Incidents, Monitoring, Enterprise, i18n
```
...
### Выполнено — Month 1 Open-Source (по состоянию на 19 апреля 2026)
...
- [x] `passport init` → `agent-manifest.json` generated + ed25519 signed
- [x] MCP Server: 8 tools functional
- [x] FRIA Generator (80% pre-filled + AI Enricher)
...
- [x] `complior scan --deep` (Semgrep/Bandit/ModelScan via Tier 2)
- [x] `complior eval <url>` (11 категорий, 680+ тестов, auto-detect адаптеры)
- [x] ISO 42001 документы (SoA generator, Risk Register generator, AI Policy template)
- [x] AI Enricher — LLM-дозаполнение FRIA и документов (scaffold + enhance)
- [x] MCP Compliance Proxy (policy engine + proxy interceptor)
- [x] Incremental scan (mtime + SHA-256 cache)
- [x] Supply Chain Audit (45+ AI SDKs, CVE detection)
- [x] Dependency Scan (5 package managers, lock file parsing)
- [x] Test Generation (JS/Python/Go from passport constraints)
...
- [ ] Hosted LLM (Mistral на Hetzner GPU, 50/мес free)
