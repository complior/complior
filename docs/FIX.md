...
L4 Паттерны кода      l4-bare-llm                   — (Info)     Информационное: рекомендация SDK     —
...
Deep (ext-semgrep)    ext-semgrep-bare-call          — (Info)     Информационное: рекомендация SDK     —
...
Scaffold-фиксы (создание файлов) не меняют исходный файл. Inline fix **сплайсит** строки прямо в исходном файле. После re-scan finding становится `pass`. **Примечание:** `l4-bare-llm` теперь `info` (не `fail`), SDK wrapping — рекомендация, не compliance fix.
...
| ~~SDK Wrapper~~ | `l4-bare-llm` | Art. 50(1) | Removed: bare-llm is info (SDK is recommendation, not fix) | — |
...
**Framework-aware генерация (disclosure):**
...
const STRATEGIES: readonly FixStrategy[] = [
  ...
  // sdkWrapperStrategy removed — bare-llm is now info (SDK is recommendation)
...
|---|-----------|------|----------|----------------|
| 1 | ~~SDK Wrapper~~ | — | `l4-bare-llm` | Removed: bare-llm is now info. @complior/sdk is a recommendation, not a compliance fix |
