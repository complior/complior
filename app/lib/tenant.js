...
const { ForbiddenError, ValidationError } = require('./errors.js');

// Tables that are scoped to an organization (tenant)
...
// Tables that are global (shared across all organizations)
...
const createTenantQuery = (db, organizationId) => {
  if (!organizationId) {
    throw new ValidationError('organizationId is required for tenant queries');
  }
  ...
  return {
    async findMany(table, { where = {}, orderBy, limit, offset } = {}) {
      ...
      if (orderBy) sql += ` ORDER BY "${orderBy.column}" ${orderBy.dir || 'ASC'}`;
    ...
    async create(table, data) {
      if (data.organizationId && data.organizationId !== organizationId) {
        throw new ForbiddenError('Cannot create resource for another organization');
      }
    ...
    async update(table, id, data) {
      if (data.organizationId && data.organizationId !== organizationId) {
        throw new ForbiddenError('Cannot transfer resource to another organization');
      }
...
module.exports = { createTenantQuery, isTenantTable, TENANT_TABLES, GLOBAL_TABLES };
