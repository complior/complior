...
- **Branch model: `feature/*` → `dev` → `main`**
  ...
  - `dev` — рабочая интеграционная ветка, сюда мержатся готовые фичи
  - `main` — релизная ветка (tag `v*` → release pipeline)
- **Merge — ТОЛЬКО user.** Ни один агент не мержит ни в dev, ни в main. PR создаёт architect, мержит user.
