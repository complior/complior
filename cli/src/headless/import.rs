...
async fn run_import_promptfoo(file: Option<&str>, json: bool, config: &TuiConfig) -> i32 {
    ...
    let input = if let Some(path) = file { match std::fs::read_to_string(path) {
        Ok(data) => data,
        Err(e) => {
    ...
    match client.post_json("/import/promptfoo", &body).await {
        Ok(result) => {
            ...
            let score = result.get("securityScore").and_then(|s| s.get("score")).and_then(serde_json::Value::as_f64).unwrap_or(0.0);
            let grade = result.get("securityScore").and_then(|s| s.get("grade")).and_then(|v| v.as_str()).unwrap_or("?");
            let probes = result.get("probesRun").and_then(serde_json::Value::as_u64).unwrap_or(0);
            let passed = result.get("probesPassed").and_then(serde_json::Value::as_u64).unwrap_or(0);
            let failed = result.get("probesFailed").and_then(serde_json::Value::as_u64).unwrap_or(0);
            ...
            if let Some(categories) = result.get("securityScore").and_then(|s| s.get("categories")).and_then(|v| v.as_array()) {
                ...
                for cat in categories {
                    ...
                    let cat_passed = cat.get("probesPassed").and_then(serde_json::Value::as_u64).unwrap_or(0);
                    let cat_total = cat.get("probesTotal").and_then(serde_json::Value::as_u64).unwrap_or(0);
                    println!("  {cat_id:<16} {cat_score:>5.0}% {cat_passed:>8} {cat_total:>8}");
                }
            }

            if result.get("securityScore").and_then(|s| s.get("criticalCapped")).and_then(serde_json::Value::as_bool).unwrap_or(false) {
                println!();
