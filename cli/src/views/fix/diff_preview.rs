...
pub(super) fn render_diff_preview(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let staged_items: Vec<(&FixableItem, &Finding)> = fix
        ...
        .filter_map(|item| scan.findings.get(item.finding_index).map(|f| (item, f)))
        .collect();

    // Summary header
    let total_impact: i32 = staged_items
        .iter()
        .map(|(item, _)| item.predicted_impact)
        .sum();
    let files_modified = staged_items
        .iter()
        .filter(|(item, _)| item.file_path.is_some())
        .count();
    let files_created = staged_items
        .iter()
        .filter(|(item, _)| item.file_path.is_none())
        .count();
    ...
    if let Some(ctx) = &finding.code_context {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                format!(
                    " {}",
                    "\u{2500}".repeat(w.saturating_sub(20 + file_path.len()))
                ),
                Style::default().fg(t.accent),
            ),
        ]));
    ...
    if let Some(diff) = &finding.fix_diff {
        ...
        if let Some(import) = &diff.import_line {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(import.clone(), Style::default().fg(t.diff_added)),
            ]));
        }
    } else if let Some(fix_text) = &finding.fix {
...
fn render_type_b(
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!(
                " {}",
                "\u{2500}".repeat(w.saturating_sub(16 + inferred_path.len()))
            ),
            Style::default().fg(t.diff_added),
        ),
    ]));
...
fn render_type_c(
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!(
                " {}",
                "\u{2500}".repeat(w.saturating_sub(16 + file_path.len()))
            ),
            Style::default().fg(t.zone_yellow),
        ),
    ]));
